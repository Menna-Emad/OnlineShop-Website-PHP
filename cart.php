<?php
$title = "Cart";
include_once "layouts/header.php";
include_once "layouts/nav.php";
include_once "app/middleware/auth.php";
include_once "app/models/Cart.php";
include_once "app/models/Product.php";



//cart->product_id,product_name,quantity,price,user_id
$cartObject = new Cart;
$cartResult = $cartObject->read();
$cartTotal = $cartObject->gettotalPrice();

if (isset($_POST['product_id'])) {
    $cartRemove = $cartObject->Delete();
}
?>





<!-- Cart Start -->
<div class="container-fluid">
    <div class="row px-xl-5">
        <div class="col-lg-8 table-responsive mb-5">
            <table class="table table-light table-borderless table-hover text-center mb-0">
                <thead class="thead-dark">
                    <tr>
                        <th>Products</th>
                        <th>Price</th>
                        <th>Quantity</th>
                        <th>Total</th>
                        <!-- <th>Remove</th> -->
                    </tr>
                </thead>
                <tbody class="align-middle">
                    <?php
                    if ($cartResult) {
                        $cartproducts = $cartResult->fetch_all(MYSQLI_ASSOC);
                        foreach ($cartproducts as $key => $cartproduct) {

                    ?>
                            <tr>
                                <td class="align-middle"><img src="img/<?= $cartproduct['image'] ?>" alt="" style="width: 50px;"> <?=$cartproduct['name_en']?></td>
                                <td class="align-middle"><?= $cartproduct['price'] ?> EGP</td>
                                <td class="align-middle">
                                    <div class="input-group quantity mx-auto" style="width: 100px;">

                                        <input type="text" value="<?= $cartproduct['quantity'] ?>" min="1" max="10" data-decimals="0" required class="form-control form-control-sm bg-secondary border-0 text-center">

                                    </div>
                                </td>
                                <td class="align-middle"><?= $cartproduct['totalprice'] ?></td>
                                <form method="post">
                                    <input type="hidden" name="product_id" value="<?= $cartproduct['product_id'] ?>">
                                    <td class="align-middle"><button type="submit" class="btn btn-sm btn-danger"><i class="fa fa-times"></i></button></td>
                                </form>
                            </tr>
                        <?php
                        }
                    } else { ?>
                        <tr>
                            <td colspan="5" style="text-align:center;">You have no products added in your Shopping Cart</td>
                        </tr>
                    <?php }
                    ?>
                </tbody>
            </table>
        </div>
        <div class="col-lg-4">
            <form class="mb-30" action="">
                <div class="input-group">
                    <input type="text" class="form-control border-0 p-4" placeholder="Coupon Code">
                    <div class="input-group-append">
                        <button class="btn btn-primary">Apply Coupon</button>
                    </div>
                </div>
            </form>
            <h5 class="section-title position-relative text-uppercase mb-3"><span class="bg-secondary pr-3">Cart Summary</span></h5>
            <div class="bg-light p-30 mb-5">
                <?php
                if ($cartTotal) {
                    $cartsubtotals = $cartTotal->fetch_all(MYSQLI_ASSOC);
                    foreach ($cartsubtotals as $key => $cartsubtotal) {

                ?>
                        <div class="border-bottom pb-2">
                            <div class="d-flex justify-content-between mb-3">
                                <h6>Subtotal</h6>

                                <h6><?= $cartsubtotal['subtotal'] ?>EGP</h6>
                            </div>
                            <div class="d-flex justify-content-between">
                                <h6 class="font-weight-medium">Shipping</h6>
                                <h6 class="font-weight-medium">50 EGP</h6>
                            </div>
                        </div>
                        <div class="pt-2">
                            <div class="d-flex justify-content-between mt-2">
                                <h5>Total</h5>
                                <?php $carttotal=$cartsubtotal['subtotal'] +50; ?>
                                <h5><?=$carttotal?> EGP</h5>
                            </div>
                            <button class="btn btn-block btn-primary font-weight-bold my-3 py-3">Proceed To Checkout</button>
                        </div>
                <?php
                    }
                } ?>
            </div>
        </div>
    </div>
</div>
<!-- Cart End -->

<?php
include_once "layouts/footer.php";
include_once "layouts/footer-scripts.php"
?>